package com.itandcstech.shayarifeeltheemotion.commons

import com.itandcstech.shayarifeeltheemotion.R
import com.itandcstech.shayarifeeltheemotion.models.ShayariModel

/**
 * @Created by Ashif on 11-02-2026
 * Know more about author at https://ashif.nexmerce.in
 */

fun getList(): List<ShayariModel> {
    return listOf(
        ShayariModel(R.drawable.love,"Love", listOf(
            "Tumhari yaad ke saaye mere dil ke andhero mein, diye ki lau ki tarah har pal chamakte hain.",
            "Mohabbat mein jhukna koi gunah nahi, chupta wahi hai jisme noor hota hai.",
            "Ishq wo nahi jo tujhe mera kar de, ishq wo hai jo tujhe kisi aur ka hone na de.",
            "Tere muskurane ka asar kuch aisa hua, ke hum khud ko bhool kar tere ho gaye.",
            "Dhadkano ko bhi raasta mil gaya, jab se tum meri zindagi mein aaye ho.",
            "Log puchte hain mohabbat kya hai, hum tumhara naam lekar muskura dete hain.",
            "Zindagi ke safar mein tera sath chahiye, bas thodi si hansi aur tera hath chahiye.",
            "Kaun kehta hai ishq mukkamal nahi hota, humne toh har saans mein unhe paya hai.",
            "Dil ki baatein dil hi jaane, tum bin ye duniya virani lage.",
            "Nazron se jab nazrein milti hain, tabhi toh dil ki dhadkan badhti hai.",
            "Ishq ek ehsaas hai jo ruh tak jata hai, ye wo phool hai jo har mausam mein khilta hai.",
            "Tum meri wo khwaish ho, jo har roz duaon mein mangi jati hai." ,
            "Dosti wo nahi jo mita di jaye, dosti wo hai jo har mushkil mein nibhai jaye.",
            "Ek sacha dost wahi hai, jo tab sath de jab puri duniya khilaf ho.",
            "Zindagi mein dost milte hain kismat se, aur unhe nibhana parta hai dil se.",
            "Dosti ka rishta sabse pyara hai, ye wo kinara hai jo tufan mein sahara hai.",
            "Kaun kehta hai dosti barabari mein hoti hai, dosti toh bas dil ki sacchai dekhti hai.",
            "Dost ki dosti mein sukoon milta hai, har dard ki dawa ka junoon milta hai.",
            "Rishte bante hain ehsaas se, aur dosti banti hai vishwas se.",
            "Khuda ne pucha kya chahiye, maine kaha mere yaar ki hansi hamesha bani rahe.",
            "Purane dost wo sharaab hain, jo jitni purani ho utni hi nasha deti hain.",

        )),

        ShayariModel(R.drawable.friends,"Friendship", listOf(
            "Dosti wo nahi jo mita di jaye, dosti wo hai jo har mushkil mein nibhai jaye.",
            "Ek sacha dost wahi hai, jo tab sath de jab puri duniya khilaf ho.",
            "Zindagi mein dost milte hain kismat se, aur unhe nibhana parta hai dil se.",
            "Dosti ka rishta sabse pyara hai, ye wo kinara hai jo tufan mein sahara hai.",
            "Kaun kehta hai dosti barabari mein hoti hai, dosti toh bas dil ki sacchai dekhti hai.",
            "Dost ki dosti mein sukoon milta hai, har dard ki dawa ka junoon milta hai.",
            "Rishte bante hain ehsaas se, aur dosti banti hai vishwas se.",
            "Khuda ne pucha kya chahiye, maine kaha mere yaar ki hansi hamesha bani rahe.",
            "Purane dost wo sharaab hain, jo jitni purani ho utni hi nasha deti hain.",
            "Dost wo hai jo aapki aankhon mein aansu dekh kar kahe, 'Ruk main abhi aata hoon'.",
            "Sachi dosti mein 'thank you' aur 'sorry' nahi hota, bas ek dusre ka sath hota hai.",
            "Doston ke bina zindagi adhuri hai, dosti hi har khushi ki manjuri hai.",
        )),

        ShayariModel(R.drawable.best,"Happiness", listOf(
            "Khushi unhe nahi milti jo apne sharton pe jeete hain, balki unhe milti hai jo dusron ki khushi mein shamil hote hain.",
            "Zindagi chhoti hai, har pal mein muskurana seekho.",
            "Khushiyan banto toh badhti hain, gham banto toh ghat-te hain.",
            "Aaj ki khushi kal ki yaad ban jati hai, isliye jee bhar ke jee lo.",
            "Muskurahat wo tohfa hai jo bina mange mil jata hai.",
            "Dil khush hai toh duniya haseen hai.",
            "Chhoti chhoti baaton mein khushiyan dhundo, sukoon mil jayega.",
            "Khushi ki talaash mein bhatakne se behtar hai, jo hai usme khush raho.",
            "Zindagi haseen hai isse pyar karo, har raat ki subah ka intezar karo.",
            "Jab dil mein sukoon hota hai, tabhi asli khushi milti hai.",
            "Haste raho kyunki rone se dukh kam nahi hota.",
            "Khushi ka rasta hamare andar se hi guzarta hai."
        )),

        ShayariModel(R.drawable.attitude,"Sadness", listOf(
            "Kabhi kabhi rona bhi zaroori hota hai, dil ka bojh halka karne ke liye.",
            "Udasi ka koi mausam nahi hota, ye toh bas dil ki ek halat hai.",
            "Aansu wo shabd hain jo dil keh nahi pata.",
            "Tanhai mein aksar purani yaadein rula deti hain.",
            "Dard tab hota hai jab apna hi koi paraya ban jaye.",
            "Khamoshi ka matlab har baar inkar nahi hota, kabhi kabhi dard bhi hota hai.",
            "Zindagi mein har koi khush nahi hota, kuch log muskurahat ke peeche dard chupate hain.",
            "Dil tutne ki awaaz nahi hoti, par dard bahut hota hai.",
            "Waqt har zakhm ko bhar deta hai, par nishan reh jate hain.",
            "Umeed jab tut-ti hai, toh insaan andar se bikhar jata hai.",
            "Aaj fir dil ne ek tamanna ki, par kismat ne fir wahi purani kahani dohrai.",
            "Gham is baat ka nahi ki tum mil na sake, gham is baat ka hai ki hum khud ko kho bethe."
        )),

        ShayariModel(R.drawable.goodnight,"Hope", listOf(
            "Andheri raat ke baad hi haseen sawera hota hai.",
            "Umeed pe hi duniya kayam hai, kabhi haar mat maano.",
            "Girkar sambhalne wale ko hi baazigar kehte hain.",
            "Har mushkil ke baad aasaani hai, bas thoda sabr rakho.",
            "Zindagi ki raahon mein dhoop toh hogi, par himmat mat harna.",
            "Ek naya din, ek nayi umeed lekar aata hai.",
            "Sapne wahi dekhte hain jinme unhe pura karne ka junoon hota hai.",
            "Hausla mat har, abhi toh pura asmaan baaki hai.",
            "Umeed ka diya kabhi bujhne mat dena, yahi rasta dikhayega.",
            "Waqt badalta hai, aaj bura hai toh kal accha bhi aayega.",
            "Hausle ke tarkash mein koshish ka wo teer zinda rakho.",
            "Haar ke baad hi jeet ka maza aata hai."
        ))

        // Add more categories following this pattern...
    )
}